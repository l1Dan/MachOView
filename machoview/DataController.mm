...
- (void)loadAttributesFromFile:(FILE *)pFile {
    ...
    while (numAttributes-- > 0) {
        ...
        switch (keyOrdinal) {
            case MVUnderlineAttributeOrdinal:
                _attributes[MVUnderlineAttributeName] = [self readStringFromFile:pFile];
                break;
            case MVCellColorAttributeOrdinal:
                _attributes[MVCellColorAttributeName] = [self readColorFromFile:pFile];
                break;
            case MVTextColorAttributeOrdinal:
                _attributes[MVTextColorAttributeName] = [self readColorFromFile:pFile];
                break;
            case MVMetaDataAttributeOrdinal:
                _attributes[MVMetaDataAttributeName] = [self readStringFromFile:pFile];
                break;
            default:
                NSLog(@"warning: unknown attribute key");
        }
...
- (void)createMachOLayout:(MVNode *)node
    ...
    [node setCaption:[NSString stringWithFormat:@"%@ (%@)",
