...
//-----------------------------------------------------------------------------
- (id)initWithData:(NSString *)col0 col1:(NSString *)col1 col2:(NSString *)col2 col3:(NSString *)col3 {
    if (self = [super init]) {
	offsetStr = col0;
	dataStr = col1;
	descriptionStr = col2;
...
//-----------------------------------------------------------------------------
+ (MVColoumns *)coloumnsWithData:(NSString *)col0 col1:(NSString *)col1 col2:(NSString *)col2 col3:(NSString *)col3 {
    return [[MVColoumns alloc] initWithData:col0 col1:col1 col2:col2 col3:col3];
}
...
- (void)loadAttributesFromFile:(FILE *)pFile {
	...
	switch (keyOrdinal) {
	    case MVUnderlineAttributeOrdinal:
		[_attributes setObject:[self readStringFromFile:pFile] forKey:MVUnderlineAttributeName];
		break;
	    case MVCellColorAttributeOrdinal:
		[_attributes setObject:[self readColorFromFile:pFile] forKey:MVCellColorAttributeName];
		break;
	    case MVTextColorAttributeOrdinal:
		[_attributes setObject:[self readColorFromFile:pFile] forKey:MVTextColorAttributeName];
		break;
	    case MVMetaDataAttributeOrdinal:
		[_attributes setObject:[self readStringFromFile:pFile] forKey:MVMetaDataAttributeName];
		break;
	    default:
		NSLog(@"warning: unknown attribute key");
	}
...
- (void)insertRowWithOffset:(uint32_t)offset col0:(id)col0 col1:(id)col1 col2:(id)col2 col3:(id)col3 {
    ...
    row.coloumns = [MVColoumns coloumnsWithData:col0 col1:col1 col2:col2 col3:col3];
    row.offset = offset;
...
- (void)appendRow:(id)col0 col1:(id)col1 col2:(id)col2 col3:(id)col3 {
    [self insertRowWithOffset:0 col0:col0 col1:col1 col2:col2 col3:col3];
}
...
//----------------------------------------------------------------------------
- (NSMutableDictionary *)attributesWithPairs:(id)firstArg args:(va_list)args {
...
- (void)setAttributes:(id)firstArg, ... {
    ...
    NSMutableDictionary *attributes = [self attributesWithPairs:firstArg args:args];
    va_end(args);
...
//----------------------------------------------------------------------------
- (void)setAttributesForRowIndex:(NSUInteger)index firstArg:(NSString *)firstArg, ... {
    va_list args;
    va_start(args, firstArg);
    NSMutableDictionary *attributes = [self attributesWithPairs:firstArg args:args];
    va_end(args);
...
//----------------------------------------------------------------------------
- (void)setAttributesFromRowIndex:(NSUInteger)index firstArg:(NSString *)firstArg, ... {
    va_list args;
    va_start(args, firstArg);
    NSDictionary *attributes = [self attributesWithPairs:firstArg args:args];
    va_end(args);
...
- (void)createMachOLayout:(MVNode *)node
    ...
    node.caption = [NSString stringWithFormat:@"%@ (%@)",
					      mach_header->filetype == MH_OBJECT ? @"Object " : mach_header->filetype == MH_EXECUTE ? @"Executable " : mach_header->filetype == MH_FVMLIB ? @"Fixed VM Shared Library" : mach_header->filetype == MH_CORE ? @"Core" : mach_header->filetype == MH_PRELOAD ? @"Preloaded Executable" : mach_header->filetype == MH_DYLIB ? @"Shared Library " : mach_header->filetype == MH_DYLINKER ? @"Dynamic Link Editor" : mach_header->filetype == MH_BUNDLE ? @"Bundle" : mach_header->filetype == MH_DYLIB_STUB ? @"Shared Library Stub" : mach_header->filetype == MH_DSYM ? @"Debug Symbols" : mach_header->filetype == MH_KEXT_BUNDLE ? @"Kernel Extension" : @"?????",
...
- (void)createMachO64Layout:(MVNode *)node
    ...
    node.caption = [NSString stringWithFormat:@"%@ (%@)",
					      mach_header_64->filetype == MH_OBJECT ? @"Object " : mach_header_64->filetype == MH_EXECUTE ? @"Executable " : mach_header_64->filetype == MH_FVMLIB ? @"Fixed VM Shared Library" : mach_header_64->filetype == MH_CORE ? @"Core" : mach_header_64->filetype == MH_PRELOAD ? @"Preloaded Executable" : mach_header_64->filetype == MH_DYLIB ? @"Shared Library " : mach_header_64->filetype == MH_DYLINKER ? @"Dynamic Link Editor" : mach_header_64->filetype == MH_BUNDLE ? @"Bundle" : mach_header_64->filetype == MH_DYLIB_STUB ? @"Shared Library Stub" : mach_header_64->filetype == MH_DSYM ? @"Debug Symbols" : mach_header_64->filetype == MH_KEXT_BUNDLE ? @"Kernel Extension" : @"?????",
