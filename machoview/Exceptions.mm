...
((format & 0xf) == DW_EH_PE_udata2 || (format & 0xf) == DW_EH_PE_sdata2) ? sizeof(uint16_t) : ((format & 0xf) == DW_EH_PE_udata4 || (format & 0xf) == DW_EH_PE_sdata4) ? sizeof(uint32_t) : ((format & 0xf) == DW_EH_PE_udata8 || (format & 0xf) == DW_EH_PE_sdata8) ? sizeof(uint64_t) : ((format & 0xf) == DW_EH_PE_uleb128 || (format & 0xf) == DW_EH_PE_sleb128) ? 0 : ([self is64bit] == NO) ? sizeof(uint32_t) : sizeof(uint64_t)

//-----------------------------------------------------------------------------
#define READ_USE_ENCODING(format, range, hexstr) \
((format & 0xf) == DW_EH_PE_udata2 || (format & 0xf) == DW_EH_PE_sdata2) ? [dataController read_uint16:range lastReadHex:&hexstr] : ((format & 0xf) == DW_EH_PE_udata4 || (format & 0xf) == DW_EH_PE_sdata4) ? [dataController read_uint32:range lastReadHex:&hexstr] : ((format & 0xf) == DW_EH_PE_udata8 || (format & 0xf) == DW_EH_PE_sdata8) ? [dataController read_uint64:range lastReadHex:&hexstr] : (format & 0xf) == DW_EH_PE_uleb128 ? [dataController read_uleb128:range lastReadHex:&hexstr] : (format & 0xf) == DW_EH_PE_sleb128 ? [dataController read_sleb128:range lastReadHex:&hexstr] : ([self is64bit] == NO) ? [dataController read_uint32:range lastReadHex:&hexstr] : [dataController read_uint64:range lastReadHex:&hexstr]
...
- (MVNode *)createCFINode:(MVNode *)parent
    ...
    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"CIE Length" col3:[NSString stringWithFormat:@"%u", CIE_length]];
    ...
    uint32_t CIE_ID = [dataController read_uint32:range lastReadHex:&lastReadHex];
    CIE_length -= range.length;
    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"CIE ID" col3:[NSString stringWithFormat:@"%u", CIE_ID]];

    //Version (Required)
    // Version assigned to the call frame information structure. This value shall be 1.
    uint8_t CIE_version = [dataController read_uint8:range lastReadHex:&lastReadHex];
    CIE_length -= range.length;
    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"CIE Version" col3:[NSString stringWithFormat:@"%u", CIE_version]];
    ...
    NSString *CIE_augmentationStr = [dataController read_string:range lastReadHex:&lastReadHex];
    CIE_length -= range.length;
    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Augmentation String" col3:CIE_augmentationStr];
    ...
    uint64_t CIE_codeAlignFactor = [dataController read_uleb128:range lastReadHex:&lastReadHex];
    CIE_length -= range.length;
    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Code Alignment Factor" col3:[NSString stringWithFormat:@"%qu", CIE_codeAlignFactor]];
    ...
    int64_t CIE_dataAlignFactor = [dataController read_sleb128:range lastReadHex:&lastReadHex];
    CIE_length -= range.length;
    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Data Alignment Factor" col3:[NSString stringWithFormat:@"%qd", CIE_dataAlignFactor]];

    //Return Address Register    (Required)
    uint8_t CIE_returnAddressRegister = [dataController read_uint8:range lastReadHex:&lastReadHex];
    CIE_length -= range.length;
    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Return Address Register" col3:[NSString stringWithFormat:@"0x%X", CIE_returnAddressRegister]];
    ...
    if ([CIE_augmentationStr rangeOfString:@"z"].location != NSNotFound) {
        ...
        uint64_t CIE_augmentationLength = [dataController read_uleb128:range lastReadHex:&lastReadHex];
        CIE_length -= range.length;
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Augmentation Length" col3:[NSString stringWithFormat:@"%qu", CIE_augmentationLength]];
        ...
        for (NSUInteger strIndex = 1; strIndex < [CIE_augmentationStr length]; ++strIndex) {
            switch ([CIE_augmentationStr characterAtIndex:strIndex]) {
                    ...
                    // The size of the LSDA pointer is specified by the pointer encoding used.
                {
                    ...
                    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"LSDA Encoding in FDE" col3:[self getNameForEncoding:LSDA_encoding]];
                }
                    break;
                    ...
                    // The size of the personality routine pointer is specified by the pointer encoding used.
                {
                    ...
                    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Personality Encoding" col3:[self getNameForEncoding:PR_encoding]];

                    uint64_t PR_offset = READ_USE_ENCODING(PR_encoding, range, lastReadHex);
                    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                                       col1:lastReadHex
                                       col2:@"Personality Routine"
                                       col3:[self is64bit] == NO ? [self guessSymbolUsingEncoding:PR_encoding atOffset:range.location withValue:(uint32_t &) PR_offset]
                    ...
                    // If present, The Augmentation Data shall include a 1 byte argument that represents the pointer encoding for the address pointers used in the FDE.
                {
                    ...
                    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Pointer Encoding in FDE" col3:[self getNameForEncoding:Pointer_encoding]];
                }
                    break;
    ...
    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Initial Instructions" col3:@""];
    ...
    if (Pointer_encoding != DW_EH_PE_omit)
        while (NSMaxRange(range) - location < length) {
            ...
            [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"FDE Length" col3:[NSString stringWithFormat:@"%u", FDE_length]];
            ...
            uint32_t FDE_CIEvalue = [dataController read_uint32:range lastReadHex:&lastReadHex];
            ...
            uint64_t FDE_CIEpointer = ([self is64bit] == NO
                    ...
                    : [self fileOffsetToRVA64:range.location]) -
                    FDE_CIEvalue;

            [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                               col1:lastReadHex
                               col2:@"CIE Pointer"
                               col3:[self is64bit] == NO
                                       ? [self findSymbolAtRVA:(uint32_t) FDE_CIEpointer] : [self findSymbolAtRVA64:FDE_CIEpointer]];

            //PC Begin    (Required)
            // An encoded constant that indicates the address of the initial location associated with this FDE.
            uint64_t PCBegin_addr = READ_USE_ENCODING(Pointer_encoding, range, lastReadHex);
            FDE_length -= range.length;
            [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                               col1:lastReadHex
                               col2:@"PC Begin" col3:[self is64bit] == NO
            ...
            uint64_t FDE_PCRange = READ_USE_ENCODING(Pointer_encoding, range, lastReadHex);
            FDE_length -= range.length;
            [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"PC Range" col3:[NSString stringWithFormat:@"%qu", FDE_PCRange]];

            if ([CIE_augmentationStr rangeOfString:@"z"].location != NSNotFound) {
                //Augmentation Data Length    (Optional)
                // An unsigned LEB128 encoded value indicating the length in bytes of the Augmentation Data.
                // This field is only present if the Augmentation String in the associated CIE contains the character 'z'.
                uint64_t FDE_augmentationLength = [dataController read_uleb128:range lastReadHex:&lastReadHex];
                FDE_length -= range.length;
                [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Augmentation Length" col3:[NSString stringWithFormat:@"%qu", FDE_augmentationLength]];
                ...
                if (FDE_augmentationLength > 0) // LSDA is present
                {
                    ...
                    // This field is only present if the Augmentation String in the associated CIE contains the character 'z'.
                    uint64_t LSDA_addr = READ_USE_ENCODING(LSDA_encoding, range, lastReadHex);
                    FDE_length -= range.length;
                    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                                       col1:lastReadHex
                                       col2:@"LSDA"
            ...
            [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Call Frame Instructions" col3:@""];
...
- (MVNode *)createLSDANode:(MVNode *)parent
    ...
    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"@LPStart format" col3:[self getNameForEncoding:LPStartFormat]];

    uint8_t typeTableFormat = [dataController read_uint8:range lastReadHex:&lastReadHex];
    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"@TType format" col3:[self getNameForEncoding:typeTableFormat]];
    ...
    if (typeTableFormat != DW_EH_PE_omit) {
        ...
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Type Table Base" col3:[self is64bit] == NO ? [NSString stringWithFormat:@"0x%X", [self fileOffsetToRVA:typeTableBaseLocation]] : [NSString stringWithFormat:@"0x%qX", [self fileOffsetToRVA64:typeTableBaseLocation]]];
    }

    uint8_t callSiteFormat = [dataController read_uint8:range lastReadHex:&lastReadHex];
    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"call-site format" col3:[self getNameForEncoding:callSiteFormat]];

    uint64_t callSiteTableLength = [dataController read_uleb128:range lastReadHex:&lastReadHex];
    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"call-site table length" col3:[NSString stringWithFormat:@"%qu", callSiteTableLength]];
    ...
    do {
        ...
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"region start" col3:[NSString stringWithFormat:@"0x%qX", eh_frame_begin + regionStart]];

        uint64_t regionLength = READ_USE_ENCODING(callSiteFormat, range, lastReadHex);
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"length" col3:[NSString stringWithFormat:@"%qu", regionLength]];

        uint64_t landingPad = READ_USE_ENCODING(callSiteFormat, range, lastReadHex);
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"landing pad" col3:landingPad == 0 ? @"0x0" : [NSString stringWithFormat:@"0x%qX", eh_frame_begin + landingPad]];

        uint64_t action = [dataController read_uleb128:range lastReadHex:&lastReadHex];
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"action" col3:[NSString stringWithFormat:@"%qu", action]];
    ...
    if (typeTableFormat != DW_EH_PE_omit) {
        ...
        while (actions.empty() == false) {
            ...
            [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Type Filter" col3:[NSString stringWithFormat:@"%qd", index]];
            ...
            int64_t nextAction = [dataController read_sleb128:range lastReadHex:&lastReadHex];
            currentAction += range.length;
            [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Next Action" col3:[NSString stringWithFormat:@"%qd", nextAction]];
        ...
        for (IndexSet::reverse_iterator iter = typeIndexes.rbegin(); iter != typeIndexes.rend(); ++iter) {
            ...
            [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                               col1:lastReadHex
                               col2:@"Type Info" col3:[self is64bit] == NO ? [self guessSymbolUsingEncoding:typeTableFormat atOffset:range.location withValue:(uint32_t &) typeInfo]
        ...
        while (NSMaxRange(range) < location + length) {
            int64_t index = [dataController read_sleb128:range lastReadHex:&lastReadHex];
            [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Exception Spec" col3:[NSString stringWithFormat:@"%lld", index]];
        }
...
- (MVNode *)createUnwindInfoHeaderNode:(MVNode *)parent
    ...
    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Unwind Section Version" col3:version == UNWIND_SECTION_VERSION ? @"UNWIND_SECTION_VERSION" : [NSString stringWithFormat:@"%u", version]];
    ...
    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                       col1:lastReadHex
                       col2:@"Common Enc Array Sect Offset"
                       col3:[self is64bit] == NO ? [self findSymbolAtRVA:[self fileOffsetToRVA:range.location] + commonEncodingsArraySectionOffset]
    ...
    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Common Enc Array Count" col3:[NSString stringWithFormat:@"%u", commonEncodingsArrayCount]];

    uint32_t personalityArraySectionOffset = [dataController read_uint32:range lastReadHex:&lastReadHex];
    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                       col1:lastReadHex
                       col2:@"Personality Array Sect Offset"
                       col3:[self is64bit] == NO ? [self findSymbolAtRVA:[self fileOffsetToRVA:range.location] + personalityArraySectionOffset]
    ...
    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Personality Array Count" col3:[NSString stringWithFormat:@"%u", personalityArrayCount]];

    uint32_t indexSectionOffset = [dataController read_uint32:range lastReadHex:&lastReadHex];
    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                       col1:lastReadHex
                       col2:@"Index Section Offset" col3:[self is64bit] == NO ? [self findSymbolAtRVA:[self fileOffsetToRVA:range.location] + indexSectionOffset]
    ...
    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Index Count" col3:[NSString stringWithFormat:@"%u", indexCount]];
