...
- (MVNode *)createPointersNode:(MVNode *)parent
    ...
    while (NSMaxRange(range) < location + length) {
        ...
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Pointer" col3:symbolName];
...
- (MVNode *)createCStringsNode:(MVNode *)parent
    ...
    while (NSMaxRange(range) < location + length) {
        ...
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:[NSString stringWithFormat:@"CString (length:%lu)", [symbolName length]] col3:symbolName];
        [node.details setAttributes:MVMetaDataAttributeName, symbolName, nil];
...
- (MVNode *)createLiteralsNode:(MVNode *)parent
    ...
    while (NSMaxRange(range) < location + length) {
        ...
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Floating Point Number" col3:literalStr];
...
- (MVNode *)createIndPointersNode:(MVNode *)parent
    ...
    while (NSMaxRange(range) < location + length) {
        ...
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Indirect Pointer" col3:symbolName];
        [node.details setAttributes:MVMetaDataAttributeName, symbolName, nil];
    }
...
- (MVNode *)createIndStubsNode:(MVNode *)parent
    ...
    while (NSMaxRange(range) < location + length) {
        ...
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Indirect Stub" col3:symbolName];
        [node.details setAttributes:MVMetaDataAttributeName, symbolName, nil];
    }
...
static AsmFootPrint const hybridStubHelperX86 =
{
    ...
    {1, 0x68},
    GAP(4), // pushl   $foo$lazy_ptr
    {1, 0xE9},
    GAP(4),    // jmp     dyld_hybrid_stub_binding_helper
    {1, 0x90}, // nop
};
...
static AsmFootPrint const hybridStubHelperHelperX86 =
{
    ...
    {1, 0xE9},
    GAP(4),               // jmpl    dyld_stub_binding_helper
    {3, 0x83, 0xC4, 0x04}, // addl    $0x04,%esp
    {1, 0x68},
    GAP(4), // pushl   imageloadercache
    {2, 0xFF, 0x25},
    GAP(4), // jmp     *_fast_lazy_bind(%rip)
};
...
static AsmFootPrint const classicStubHelperX86_64 =
{
    ...
    {1, 0xE9},
    GAP(4), // jmp    dyld_stub_binding_helper
};
...
static AsmFootPrint const hybridStubHelperX86_64 =
{
    ...
    {3, 0x4C, 0x8D, 0x1D},
    GAP(4), // lea    foo$lazy_ptr(%rip),%r11
    {1, 0xE9},
    GAP(4),    // jmp    helper-helper
    {1, 0x90}, // nop
};
...
- (MVNode *)createStubHelperNode:(MVNode *)parent
    ...
    if ([self matchAsmAtOffset:range.location
        ...
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"cmpl  $0x00,_fast_lazy_bind" col3:[self findSymbolAtRVA:address]];

        [dataController read_bytes:range length:2 lastReadHex:&lastReadHex];
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"jne  $0x0D" col3:@""];
        
        [dataController read_bytes:range length:4 lastReadHex:&lastReadHex];
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"movl  %eax,4(%esp)" col3:@""];
        
        [dataController read_bytes:range length:1 lastReadHex:&lastReadHex];
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"popl  %eax" col3:@""];
        
        [dataController read_bytes:range length:3 lastReadHex:&lastReadHex];
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"xchgl  (%esp),%eax" col3:@""];
        ...
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"jmpl  dyld_stub_binding_helper" col3:[self findSymbolAtRVA:address]];
        
        [dataController read_bytes:range length:3 lastReadHex:&lastReadHex];
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"addl  $0x04,%esp" col3:@""];
        
        data = [dataController read_bytes:range length:5 lastReadHex:&lastReadHex];
        address = *(uint32_t *)((uint8_t *)data.bytes + 1);
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"pushl  imageloadercache" col3:[self findSymbolAtRVA:address]];
        
        data = [dataController read_bytes:range length:6 lastReadHex:&lastReadHex];
        address = *(uint32_t *)((uint8_t *)data.bytes + 2);
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"jmp  *_fast_lazy_bind(%rip)" col3:[self findSymbolAtRVA:address]];
        ...
        data = [dataController read_bytes:range length:6 lastReadHex:&lastReadHex];
        address = *(uint32_t *)((uint8_t *)data.bytes + 2);
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"jmp  *_fast_lazy_bind" col3:[self findSymbolAtRVA:address]];
        
        [dataController read_bytes:range length:1 lastReadHex:&lastReadHex];
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"nop" col3:@""];
...
- (MVNode *)createTextNode:(MVNode *)parent
    ...
    for (size_t i = 0; i < disasm_count; i++) {
        ...
        [node.details appendRow:[NSString stringWithFormat:@"%.8X", fileOffset] col1:lastReadHex col2:asm_string col3:@""];
