...
#define MATCHASM(_pattern)                     \
    (offset + sizeof(_pattern) < dataLength && \
     [self matchAsmAtOffset:offset             \
           asmFootPrint:_pattern           \
          lineCount:sizeof(_pattern) / FOOTPRINT_STRIDE])
...
static AsmFootPrint const SDK104Target104X86v1 =
        {
                ...
                {1, 0xE8},
                GAP(4),               // call    sub_120AD8
                {3, 0x8B, 0x45, 0x08}, // mov     eax, [ebp+arg_0]
                {2, 0x89, 0x83},
                GAP(4),               // mov     ds:(_NXArgc - 2C28h)[ebx], eax
                {3, 0x8B, 0x45, 0x0C}, // mov     eax, [ebp+arg_4]
                {2, 0x89, 0x83},
                GAP(4),               // mov     ds:(_NXArgv - 2C28h)[ebx], eax
                {3, 0x8B, 0x45, 0x10}, // mov     eax, [ebp+arg_8]
                {2, 0x89, 0x83},
                GAP(4), // mov     ds:(_environ - 2C28h)[ebx], eax
                {2, 0x8B, 0x83},
                GAP(4),         // mov     eax, ds:(_mach_init_routine_ptr - 2C28h)[ebx]
                ...
                //loc_2C51:
                {2, 0x8B, 0x83},
                GAP(4),         // mov     eax, ds:(__cthread_init_routine_ptr - 2C28h)[ebx]
                {2, 0x8B, 0x00}, // mov     eax, [eax]
                {2, 0x85, 0xC0}, // test    eax, eax
                ...
                //loc_2C8D:
                {2, 0x8B, 0x83},
                GAP(4),                     // mov     eax, ds:(_errno_ptr - 2C28h)[ebx]
                ...
                {2, 0x89, 0x83},
                GAP(4),         // mov     ds:(___progname - 2C28h)[ebx], eax
                {2, 0xEB, 0x06}, // jmp     short loc_2CCB
                // ---------------------------------------------------------------------------

                //loc_2CC5:
                {2, 0x89, 0xBB},
                GAP(4), // mov     ds:(___progname - 2C28h)[ebx], edi
                ...
                {4, 0x89, 0x44, 0x24, 0x04}, // mov     [esp+4], eax
                {2, 0x8D, 0x83},
                GAP(4),                   // lea     eax, (a__dyld_make_de - 2D25h)[ebx] ; "__dyld_make_delayed_module_initializer_"...
                {3, 0x89, 0x04, 0x24},           // mov     [esp], eax
                ...
                {2, 0xFF, 0x25},
                GAP(4), // jmp     ds:dword_150010
                // ---------------------------------------------------------------------------
                {1, 0x90}, // nop

                //sub_2D50:
                {2, 0xFF, 0x25},
                GAP(4), // jmp     ds:dword_150014
        };
...
static AsmFootPrint const SDK104Target104X86v2 =
        {
                ...
                {3, 0x8B, 0x45, 0x08}, // mov     eax, [ebp+arg_0]
                {1, 0xA3},
                GAP(4), // mov     ds:_NXArgc, eax
                {2, 0x89, 0x3D},
                GAP(4), // mov     ds:_NXArgv, edi
                {2, 0x89, 0x1D},
                GAP(4),         // mov     ds:_environ, ebx
                {2, 0x8B, 0x0F}, // mov     ecx, [edi]
                {2, 0x85, 0xC9}, // test    ecx, ecx
                {2, 0x75, 0x07}, // jnz     short loc_5A
                {1, 0xB9},
                GAP(4),         // mov     ecx, offset byte_120
                ...
                {1, 0xA1},
                GAP(4),         // mov     eax, ds:_mach_init_routine_ptr
                {2, 0x8B, 0x00}, // mov     eax, [eax]
                {2, 0x85, 0xC0}, // test    eax, eax
                ...
                //loc_96:
                {1, 0xA1},
                GAP(4),         // mov     eax, ds:__cthread_init_routine_ptr
                {2, 0x8B, 0x00}, // mov     eax, [eax]
                {2, 0x85, 0xC0}, // test    eax, eax
                ...
                {4, 0x89, 0x44, 0x24, 0x04}, // mov     [esp+4], eax
                {3, 0xC7, 0x04, 0x24},
                GAP(4), // mov     dword ptr [esp], offset a__dyld_make_de ; "__dyld_make_delayed_module_initializer_"...
                {1, 0xE8},
                GAP(4),                 // call    __dyld_func_lookup
                {3, 0xFF, 0x55, 0xE0},       // call    [ebp+var_20]
                {3, 0x8D, 0x45, 0xE4},       // lea     eax, [ebp+var_1C]
                {4, 0x89, 0x44, 0x24, 0x04}, // mov     [esp+4], eax
                {3, 0xC7, 0x04, 0x24},
                GAP(4), // mov     dword ptr [esp], offset a__dyld_mod_ter ; "__dyld_mod_term_funcs"
                {1, 0xE8},
                GAP(4),               // call    __dyld_func_lookup
                {3, 0x8B, 0x45, 0xE4}, // mov     eax, [ebp+var_1C]
                {2, 0x85, 0xC0},       // test    eax, eax
...
static AsmFootPrint const SDK104Target104X86v3 =
        {
                ...
                {1, 0xB9},
                GAP(4),         // mov     ecx, offset unk_13C874
...
static AsmFootPrint const SDK104Target104X86_64 =
        {
                ...
                {2, 0x89, 0x3D},
                GAP(4), // mov     cs:_NXArgc, edi
                {3, 0x48, 0x89, 0x35},
                GAP(4), // mov     cs:_NXArgv, rsi
                {3, 0x48, 0x89, 0x15},
                GAP(4),               // mov     cs:_environ, rdx
                {3, 0x48, 0x8B, 0x0E}, // mov     rcx, [rsi]
                {3, 0x48, 0x85, 0xC9}, // test    rcx, rcx
                {2, 0x75, 0x09},       // jnz     short loc_5F
                {3, 0x48, 0x8D, 0x0D},
                GAP(4),         // lea     rcx, byte_180
                ...
                {3, 0x48, 0x8B, 0x05},
                GAP(4),               // mov     rax, cs:_mach_init_routine
                ...
                // loc_A7
                {3, 0x48, 0x8B, 0x05},
                GAP(4),               // mov     rax, cs:__cthread_init_routine
                {3, 0x48, 0x8B, 0x00}, // mov     rax, [rax]
                {3, 0x48, 0x85, 0xC0}, // test    rax, rax
                ...
                // loc_B8
                {1, 0xE8},
                GAP(4),                 // call    near ptr ___keymgr_dwarf2_register_sections
                {4, 0x48, 0x8D, 0x75, 0xD0}, // lea     rsi, [rbp+var_30]
                {3, 0x48, 0x8D, 0x3D},
                GAP(4), // lea     rdi, a__dyld_make_de ; "__dyld_make_delayed_module_initializer_"...
                {1, 0xE8},
                GAP(4),                 // call    __dyld_func_lookup
                {3, 0xFF, 0x55, 0xD0},       // call    [rbp+var_30]
                {4, 0x48, 0x8D, 0x75, 0xD8}, // lea     rsi, [rbp+var_28]
                {3, 0x48, 0x8D, 0x3D},
                GAP(4), // lea     rdi, a__dyld_mod_ter ; "__dyld_mod_term_funcs"
                {1, 0xE8},
                GAP(4),                 // call    __dyld_func_lookup
                ...
                {1, 0xE8},
                GAP(4), // call    near ptr _atexit

                // loc_EE
                {3, 0x48, 0x8B, 0x05},
                GAP(4),                     // mov     rax, cs:_errno
                ...
                {1, 0xE8},
                GAP(4),         // call    near ptr _main
                {2, 0x89, 0xC7}, // mov     edi, eax
                {1, 0xE8},
                GAP(4), // call    near ptr _exit
        };
...
static AsmFootPrint const SDK105Target104X86_64 =
        {
                ...
                {4, 0x48, 0x8D, 0x75, 0xD0}, // lea     rsi, [rbp+var_30]
                {3, 0x48, 0x8D, 0x3D},
                GAP(4), // lea     rdi, LC1        ; "__dyld_make_delayed_module_initializer_"...
                {1, 0xE8},
                GAP(4),               // call    __dyld_func_lookup
                {3, 0xFF, 0x55, 0xD0}, // call    [rbp+var_30]
                {3, 0x48, 0x8D, 0x3D},
                GAP(4),                 // lea     rdi, LC2        ; "__dyld_mod_term_funcs"
                {4, 0x48, 0x8D, 0x75, 0xD8}, // lea     rsi, [rbp+var_28]
                {1, 0xE8},
                GAP(4),                 // call    __dyld_func_lookup
                {4, 0x48, 0x8B, 0x7D, 0xD8}, // mov     rdi, [rbp+var_28]
...
static AsmFootPrint const SDK106Target104X86v2 =
        {
                ...
                {1, 0xB9},
                GAP(4),         // mov     ecx, offset byte_199DAE
                {2, 0x85, 0xC0}, // test    eax, eax
...
static AsmFootPrint const SDK109Target104X86 = {
        ...
        {4, 0x89, 0x44, 0x24, 0x04},                // mov     [esp+4], eax
        {3, 0xC7, 0x04, 0x24},
        GAP(4),                                     // mov     dword ptr [esp], offset a__dyld_make_de ;
        {1, 0xE8},
        GAP(4),                                        // call    __dyld_func_lookup
        {3, 0xFF, 0x55, 0xEC},                      // call    [ebp+var_14]
        {3, 0x8D, 0x45, 0xF0},                      // lea     eax, [ebp+var_10]
        {4, 0x89, 0x44, 0x24, 0x04},                // mov     [esp+4], eax
        {3, 0xC7, 0x04, 0x24},
        GAP(4),                                     // mov     dword ptr [esp], offset a__dyld_mod_ter ;
        {1, 0xE8},
        GAP(4),                                        // call    __dyld_func_lookup
        {3, 0x8B, 0x45, 0xF0},                      // mov     eax, [ebp+var_10]
        {2, 0x85, 0xC0},                            // test    eax, eax
