                ...
                baseAddress:(uint32_t)baseAddress // start of the section containing the relocation (image: start of the first segment)
{
    ...
    for (uint32_t nreloc = 0; nreloc < length / sizeof(struct relocation_info); ++nreloc) {
        ...
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@
         "Address" col3:[NSString stringWithFormat:@"0x%X", relocation_info
                    ? relocation_info->r_address + baseAddress
                                             : scattered_relocation_info->r_address + baseAddress]];
        
        [node.details appendRow:@"" col1:@"" col2:@"Scattered" col3:scattered_relocation_info ? @"True" : @"False"];
        ...
        if (relocation_info) {
            ...
            if (relocation_info->r_extern) {
                ...
                [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                                   col1:lastReadHex
                                   col2:@"Symbol"
                                   col3:(nlist->n_type & N_TYPE) == N_SECT ? [NSString stringWithFormat:@"0x%X (%@)", nlist->n_value, symbolName]
                                       : symbolName];
                ...
                if ((nlist->n_type & N_TYPE) == N_SECT) {
                    ...
                    if (relocAddend != 0) {
                        [node.details appendRow:@""
                                           col1:@""
                                           col2:@"Addend"
                                           col3:(int32_t)relocAddend < 0 ? [NSString stringWithFormat:@"-0x%X", -relocAddend]
            ...
            } else // r_symbolnum means section index
            {
                if (relocation_info->r_symbolnum == R_ABS) {
                    // absolute address
                    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Section" col3:@"R_ABS"];
                } else {
                    ...
                    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Section" col3:[NSString stringWithFormat:@"%u %@", relocation_info->r_symbolnum, sectionName]];
                    
                    [node.details appendRow:@"" col1:@"" col2:@"Target"  col3:(symbolName = [self findSymbolAtRVA:relocValue])];
        ...
        if (mach_header->cputype == CPU_TYPE_I386) {
            ...
            [node.details appendRow:@"" col1:@"" col2:@"Type" col3:r_type == GENERIC_RELOC_VANILLA ? @"GENERIC_RELOC_VANILLA" : r_type == GENERIC_RELOC_PAIR ? @"GENERIC_RELOC_PAIR" : r_type == GENERIC_RELOC_SECTDIFF ? @"GENERIC_RELOC_SECTDIFF" : r_type == GENERIC_RELOC_PB_LA_PTR ? @"GENERIC_RELOC_PB_LA_PTR" : r_type == GENERIC_RELOC_LOCAL_SECTDIFF ? @"GENERIC_RELOC_LOCAL_SECTDIFF" : r_type == GENERIC_RELOC_TLV ? @"GENERIC_RELOC_TLV" : @"?????"];
        } else if (mach_header->cputype == CPU_TYPE_ARM) {
            uint32_t r_type = (relocation_info ? relocation_info->r_type : scattered_relocation_info->r_type);
            
            [node.details appendRow:@"" col1:@"" col2:@"Type" col3:r_type == ARM_RELOC_VANILLA ? @"ARM_RELOC_VANILLA" : r_type == ARM_RELOC_PAIR ? @"ARM_RELOC_PAIR" : r_type == ARM_RELOC_SECTDIFF ? @"ARM_RELOC_SECTDIFF" : r_type == ARM_RELOC_LOCAL_SECTDIFF ? @"ARM_RELOC_LOCAL_SECTDIFF" : r_type == ARM_RELOC_PB_LA_PTR ? @"ARM_RELOC_PB_LA_PTR" : r_type == ARM_RELOC_BR24 ? @"ARM_RELOC_BR24" : r_type == ARM_THUMB_RELOC_BR22 ? @"ARM_THUMB_RELOC_BR22" : r_type == ARM_THUMB_32BIT_BRANCH ? @"ARM_THUMB_32BIT_BRANCH" : r_type == ARM_RELOC_HALF ? @"ARM_RELOC_HALF" : r_type == ARM_RELOC_HALF_SECTDIFF ? @"ARM_RELOC_HALF_SECTDIFF" : @"?????"];
        }
        
        if (relocation_info) {
            [node.details appendRow:@"" col1:@"" col2:@"External" col3:relocation_info->r_extern ? @"True" : @"False"];
        }
        
        [node.details appendRow: @""
                           col1:@""
                           col2:@"PCRelative"
                           col3:(relocation_info && relocation_info->r_pcrel) || (scattered_relocation_info && scattered_relocation_info->r_pcrel) ? @"True"
                               : @"False"];
        
        [node.details appendRow:@""
                           col1:@""
                           col2:@"Length"
                           col3:[NSString stringWithFormat:@"%u", relocation_info ? (1 << relocation_info->r_length) : (1 << scattered_relocation_info->r_length)]];
        
        if (scattered_relocation_info) {
            [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Value" col3:(symbolName = [self findSymbolAtRVA:scattered_relocation_info->r_value])];
            
            color = [NSColor magentaColor];
        }
        
        [node.details setAttributesFromRowIndex:bookmark firstArg:MVMetaDataAttributeName, symbolName, MVCellColorAttributeName, color, nil];
                  ...
                  baseAddress:(uint64_t)baseAddress // start of the section containing the relocation (image: start of the first segment)
{
    ...
    for (uint32_t nreloc = 0; nreloc < length / sizeof(struct relocation_info); ++nreloc) {
        ...
        // read the first half of the entry
        [dataController read_uint32:range lastReadHex:&lastReadHex];
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Address" col3:[NSString stringWithFormat:@"0x%qX", relocation_info->r_address + baseAddress]];
        ...
        if (relocation_info->r_extern) {
            ...
            [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                               col1:lastReadHex
                               col2:@"Symbol"
                               col3:(nlist_64->n_type & N_TYPE) == N_SECT ? [NSString stringWithFormat:@"0x%qX (%@)", nlist_64->n_value, symbolName]
                                   : symbolName];
            ...
            if ((mach_header_64->cputype == CPU_TYPE_X86_64 && relocation_info->r_type == X86_64_RELOC_SUBTRACTOR) ||
                ...
                if (relocLength == sizeof(uint32_t)) {
                    ...
                    if (relocAddend != 0) {
                        [node.details appendRow:@""
                                           col1:@""
                                           col2:@"Addend"
                                           col3:(int64_t)relocAddend < 0 ? [NSString stringWithFormat:@"-0x%qX", -relocAddend]
                ...
                if (relocLength == sizeof(uint32_t)) {
                    ...
                    if (mach_header_64->cputype == CPU_TYPE_X86_64) {
                        relocAddend -= (relocation_info->r_type == X86_64_RELOC_SIGNED_1 ? 1 : relocation_info->r_type == X86_64_RELOC_SIGNED_2 ? 2 : relocation_info->r_type == X86_64_RELOC_SIGNED_4 ? 4 : 0);
                    }
            ...
            if (relocation_info->r_symbolnum == R_ABS) {
                ...
                if ((mach_header_64->cputype == CPU_TYPE_X86_64 && relocation_info->r_type == X86_64_RELOC_SUBTRACTOR) ||
                    ...
                    if ((mach_header_64->cputype == CPU_TYPE_X86_64 && relocation_info->r_type == X86_64_RELOC_UNSIGNED) ||
                        ...
                        if (mach_header_64->cputype == CPU_TYPE_X86_64) {
                            relocValue -= (relocation_info->r_type == X86_64_RELOC_SIGNED_1 ? 1 : relocation_info->r_type == X86_64_RELOC_SIGNED_2 ? 2 : relocation_info->r_type == X86_64_RELOC_SIGNED_4 ? 4 : 0);
                        }
                    }
                } else if (relocLength == sizeof(uint64_t)) {
                ...
                [node.details appendRow:@"" col1:@"" col2:@"Target" col3:(symbolName = [self findSymbolAtRVA64:relocValue])];
        ...
        if (mach_header_64->cputype == CPU_TYPE_X86_64) {
            [node.details appendRow:@"" col1:@"" col2:@"Type" col3:relocation_info->r_type == X86_64_RELOC_UNSIGNED ? @"X86_64_RELOC_UNSIGNED" : relocation_info->r_type == X86_64_RELOC_SIGNED ? @"X86_64_RELOC_SIGNED" : relocation_info->r_type == X86_64_RELOC_BRANCH ? @"X86_64_RELOC_BRANCH" : relocation_info->r_type == X86_64_RELOC_GOT_LOAD ? @"X86_64_RELOC_GOT_LOAD" : relocation_info->r_type == X86_64_RELOC_GOT ? @"X86_64_RELOC_GOT" : relocation_info->r_type == X86_64_RELOC_SUBTRACTOR ? @"X86_64_RELOC_SUBTRACTOR" : relocation_info->r_type == X86_64_RELOC_SIGNED_1 ? @"X86_64_RELOC_SIGNED_1" : relocation_info->r_type == X86_64_RELOC_SIGNED_2 ? @"X86_64_RELOC_SIGNED_2" : relocation_info->r_type == X86_64_RELOC_SIGNED_4 ? @"X86_64_RELOC_SIGNED_4" : @"?????"];
        } else if (mach_header_64->cputype == CPU_TYPE_ARM64) {
            [node.details appendRow:@"" col1:@"" col2:@"Type" col3:relocation_info->r_type == ARM64_RELOC_UNSIGNED ? @"ARM64_RELOC_UNSIGNED" : relocation_info->r_type == ARM64_RELOC_SUBTRACTOR ? @"ARM64_RELOC_SUBTRACTOR" : relocation_info->r_type == ARM64_RELOC_BRANCH26 ? @"ARM64_RELOC_BRANCH26" : relocation_info->r_type == ARM64_RELOC_PAGE21 ? @"ARM64_RELOC_PAGE21" : relocation_info->r_type == ARM64_RELOC_PAGEOFF12 ? @"ARM64_RELOC_PAGEOFF12" : relocation_info->r_type == ARM64_RELOC_GOT_LOAD_PAGE21 ? @"ARM64_RELOC_GOT_LOAD_PAGE21" : relocation_info->r_type == ARM64_RELOC_GOT_LOAD_PAGEOFF12 ? @"ARM64_RELOC_GOT_LOAD_PAGEOFF12" : relocation_info->r_type == ARM64_RELOC_POINTER_TO_GOT ? @"ARM64_RELOC_POINTER_TO_GOT" : relocation_info->r_type == ARM64_RELOC_TLVP_LOAD_PAGE21 ? @"ARM64_RELOC_TLVP_LOAD_PAGE21" : relocation_info->r_type == ARM64_RELOC_TLVP_LOAD_PAGEOFF12 ? @"ARM64_RELOC_TLVP_LOAD_PAGEOFF12" : relocation_info->r_type == ARM64_RELOC_ADDEND ? @"ARM64_RELOC_ADDEND" : @"?????"];
        }
        ...
        [node.details appendRow:@"" col1:@"" col2:@"PCRelative" col3:relocation_info->r_pcrel ? @"True" : @"False"];
        
        [node.details appendRow:@"" col1:@"" col2:@"Length" col3:[NSString stringWithFormat:@"%u", relocLength]];
...
- (MVNode *)createSymbolsNode:parent
    ...
    for (uint32_t nsym = 0; nsym < length / sizeof(struct nlist); ++nsym) {
        ...
        /* print the symbol nr */
        [node.details appendRow:[NSString stringWithFormat:@"#%d", nsym] col1:@"" col2:@"" col3:@""];
        
        [dataController read_uint32:range lastReadHex:&lastReadHex];
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"String Table Index" col3:symbolName];
        
        [dataController read_uint8:range lastReadHex:&lastReadHex];
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Type" col3:@""];
        
        if (nlist->n_type & N_STAB) {
            [node.details appendRow:@"" col1:@"" col2:@"E0" col3:@"N_STAB"];
            color = [NSColor magentaColor];
        } else {
            switch (nlist->n_type & N_TYPE) {
                case N_UNDF:
                    [node.details appendRow:@"" col1:@"" col2:@"00" col3:@"N_UNDF"];
                    break;
                case N_ABS:
                    [node.details appendRow:@"" col1:@"" col2:@"02" col3:@"N_ABS"];
                    break;
                case N_SECT:
                    [node.details appendRow:@"" col1:@"" col2:@"0E" col3:@"N_SECT"];
                    break;
                case N_PBUD:
                    [node.details appendRow:@"" col1:@"" col2:@"0C" col3:@"N_PBUD"];
                    break;
                case N_INDR:
                    [node.details appendRow:@"" col1:@"" col2:@"0A" col3:@"N_INDR"];
                    break;
            }
            
            if (nlist->n_type & N_PEXT) [node.details appendRow:@"" col1:@"" col2:@"10" col3:@"N_PEXT"];
            if (nlist->n_type & N_EXT) [node.details appendRow:@"" col1:@"" col2:@"01" col3:@"N_EXT"];
        ...
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                           col1:lastReadHex
                           col2:@"Section Index"
                           col3:nlist->n_sect == NO_SECT ? @"NO_SECT"
        ...
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Description" col3:@""];
        ...
        if ((nlist->n_type & N_STAB) == 0 &&
            ...
            switch (nlist->n_desc & REFERENCE_TYPE) {
                case REFERENCE_FLAG_UNDEFINED_NON_LAZY:
                    [node.details appendRow:@"" col1:@"" col2:@"0" col3:@"REFERENCE_FLAG_UNDEFINED_NON_LAZY"];
                    break;
                case REFERENCE_FLAG_UNDEFINED_LAZY:
                    [node.details appendRow:@"" col1:@"" col2:@"1" col3:@"REFERENCE_FLAG_UNDEFINED_LAZY"];
                    break;
                case REFERENCE_FLAG_DEFINED:
                    [node.details appendRow:@"" col1:@"" col2:@"2" col3:@"REFERENCE_FLAG_DEFINED"];
                    break;
                case REFERENCE_FLAG_PRIVATE_DEFINED:
                    [node.details appendRow:@"" col1:@"" col2:@"3" col3:@"REFERENCE_FLAG_PRIVATE_DEFINED"];
                    break;
                case REFERENCE_FLAG_PRIVATE_UNDEFINED_NON_LAZY:
                    [node.details appendRow:@"" col1:@"" col2:@"4" col3:@"REFERENCE_FLAG_PRIVATE_UNDEFINED_NON_LAZY"];
                    break;
                case REFERENCE_FLAG_PRIVATE_UNDEFINED_LAZY:
                    [node.details appendRow:@"" col1:@"" col2:@"5" col3:@"REFERENCE_FLAG_PRIVATE_UNDEFINED_LAZY"];
                    break;
                default:
                    [node.details appendRow:@"" col1:@"" col2:[NSString stringWithFormat:@"%u", nlist->n_desc & REFERENCE_TYPE] col3:@"???"];
                    break;
            }
            ...
            [node.details appendRow:@""
                               col1:@""
                               col2:@"Library Ordinal"
                               col3:[NSString stringWithFormat:@"%u (%@)", libOrdinal,
                                     libOrdinal == SELF_LIBRARY_ORDINAL ? @"SELF_LIBRARY_ORDINAL" : libOrdinal == DYNAMIC_LOOKUP_ORDINAL ? @"DYNAMIC_LOOKUP_ORDINAL" : libOrdinal == EXECUTABLE_ORDINAL ? @"EXECUTABLE_ORDINAL" : [NSSTRING((uint8_t *)dylib + dylib->name.offset - sizeof(struct load_command)) lastPathComponent]]];
        }
        
        if ((nlist->n_desc & N_ARM_THUMB_DEF) == N_ARM_THUMB_DEF) [node.details appendRow:@"" col1:@"" col2:@"0008" col3:@"N_ARM_THUMB_DEF"];
        if ((nlist->n_desc & REFERENCED_DYNAMICALLY) == REFERENCED_DYNAMICALLY) [node.details appendRow:@"" col1:@"" col2:@"0010" col3:@"REFERENCED_DYNAMICALLY"];
        if ((nlist->n_desc & N_NO_DEAD_STRIP) == N_NO_DEAD_STRIP) [node.details appendRow:@"" col1:@"" col2:@"0020" col3:@"N_NO_DEAD_STRIP"];
        if ((nlist->n_desc & N_WEAK_REF) == N_WEAK_REF) [node.details appendRow:@"" col1:@"" col2:@"0040" col3:@"N_WEAK_REF"];
        
        if ((nlist->n_type & N_TYPE) == N_UNDF) {
            if ((nlist->n_desc & N_REF_TO_WEAK) == N_REF_TO_WEAK) [node.details appendRow:@"" col1:@"" col2:@"0080" col3:@"N_REF_TO_WEAK"];
        } else {
            if ((nlist->n_desc & N_WEAK_DEF) == N_WEAK_DEF) [node.details appendRow:@"" col1:@"" col2:@"0080" col3:@"N_WEAK_DEF"];
            if ((nlist->n_desc & N_SYMBOL_RESOLVER) == N_SYMBOL_RESOLVER) [node.details appendRow:@"" col1:@"" col2:@"0100" col3:@"N_SYMBOL_RESOLVER"];
        }
        ...
        if ((nlist->n_type & N_TYPE) == N_SECT) {
            [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                               col1:lastReadHex
                               col2:@"Value"
                               col3:(nlist->n_type & N_STAB) || section == NULL
            ...
            [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Value" col3:[NSString stringWithFormat:@"%u", nlist->n_value]];
        ...
        [node.details setAttributesFromRowIndex:bookmark firstArg:MVMetaDataAttributeName, symbolName,
...
- (MVNode *)createSymbols64Node:parent
    ...
    for (uint32_t nsym = 0; nsym < length / sizeof(struct nlist_64); ++nsym) {
        ...
        if (nlist_64->n_type & N_STAB) {
            ...
            if (nlist_64->n_type & N_PEXT) [node.details appendRow:@"" col1:@"" col2:@"10" col3:@"N_PEXT"];
            if (nlist_64->n_type & N_EXT) [node.details appendRow:@"" col1:@"" col2:@"01" col3:@"N_EXT"];
        ...
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                           col1:lastReadHex
                           col2:@"Section Index"
                           col3:nlist_64->n_sect == NO_SECT ? @"NO_SECT"
                               : [NSString stringWithFormat:@"%u (%s,%s)",
                                  nlist_64->n_sect,
                                  string(section_64->segname, 16).c_str(),
                                  string(section_64->sectname, 16).c_str()]];
        ...
        if ((nlist_64->n_type & N_STAB) == 0 &&
            ...
            switch (nlist_64->n_desc & REFERENCE_TYPE) {
                ...
                case REFERENCE_FLAG_PRIVATE_UNDEFINED_LAZY:
                    ...
                    break;
                default:
                    [node.details appendRow:@"" col1:@"" col2:[NSString stringWithFormat:@"%u", nlist_64->n_desc & REFERENCE_TYPE] col3:@"???"];
                    break;
            }
            ...
            [node.details appendRow:@""
                               col1:@""
                               col2:@"Library Ordinal"
                               col3:[NSString stringWithFormat:@"%u (%@)", libOrdinal, libOrdinal == SELF_LIBRARY_ORDINAL ? @"SELF_LIBRARY_ORDINAL" : libOrdinal == DYNAMIC_LOOKUP_ORDINAL ? @"DYNAMIC_LOOKUP_ORDINAL" : libOrdinal == EXECUTABLE_ORDINAL ? @"EXECUTABLE_ORDINAL" : [NSSTRING((uint8_t *)dylib + dylib->name.offset - sizeof(struct load_command)) lastPathComponent]]];
        }
        
        if ((nlist_64->n_desc & REFERENCED_DYNAMICALLY) == REFERENCED_DYNAMICALLY) [node.details appendRow:@"" col1:@"" col2:@"0010" col3:@"REFERENCED_DYNAMICALLY"];
        if ((nlist_64->n_desc & N_NO_DEAD_STRIP) == N_NO_DEAD_STRIP) [node.details appendRow:@"" col1:@"" col2:@"0020" col3:@"N_NO_DEAD_STRIP"];
        if ((nlist_64->n_desc & N_WEAK_REF) == N_WEAK_REF) [node.details appendRow:@"" col1:@"" col2:@"0040" col3:@"N_WEAK_REF"];
        if ((nlist_64->n_desc & N_WEAK_DEF) == N_WEAK_DEF) [node.details appendRow:@""
                                                                              col1:@""
                                                                              col2:@"0080"
                                                                              col3: (nlist_64->n_type & N_TYPE) == N_UNDF || (nlist_64->n_type & N_TYPE) == N_PBUD ? @"N_REF_TO_WEAK"
                                                                                  : @"N_WEAK_DEF"];
        if ((nlist_64->n_desc & N_SYMBOL_RESOLVER) == N_SYMBOL_RESOLVER) [node.details appendRow:@"" col1:@"" col2:@"0100" col3:@"N_SYMBOL_RESOLVER"];
        ...
        if ((nlist_64->n_type & N_TYPE) == N_SECT) {
            [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                               col1:lastReadHex
                               col2:@"Value"
                               col3:(nlist_64->n_type & N_STAB) || section_64 == NULL ? [NSString stringWithFormat:@"%qu", nlist_64->n_value]
                                   : nlist_64->n_value == 0 ? @"0"
            ...
            [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Value" col3:[NSString stringWithFormat:@"%qu", nlist_64->n_value]];
...
- (MVNode *)createReferencesNode:parent
    ...
    while (NSMaxRange(range) < location + length) {
        ...
        [dataController read_uint32:range lastReadHex:&lastReadHex];
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Symbol" col3:symbolName];
        
        [node.details appendRow:@"" col1:@"" col2:@"Flags" col3:@""];
        ...
        switch (dylib_reference->flags) {
            ...
            case REFERENCE_FLAG_PRIVATE_UNDEFINED_LAZY:
                ...
                break;
            default:
                [node.details appendRow:@"" col1:@"" col2:[NSString stringWithFormat:@"%u", dylib_reference->flags] col3:@"???"];
                break;
        }
    ...
    for (uint32_t nindsym = 0; nindsym < length / sizeof(uint32_t); ++nindsym) {
        ...
        while (--nsect > 0) {
            ...
            if ((indirectIndex & (INDIRECT_SYMBOL_LOCAL | INDIRECT_SYMBOL_ABS)) == 0) {
                ...
                [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Symbol" col3:@""];
                
                switch (indirectIndex) {
                    case INDIRECT_SYMBOL_LOCAL: {
                        [node.details appendRow:@"" col1:@"" col2:@"80000000" col3:@"INDIRECT_SYMBOL_LOCAL"];
                        color = [NSColor magentaColor];
                        ...
                        [node.details appendRow:@"" col1:@"" col2:@"Target" col3:(symbolName = [self findSymbolAtRVA:targetAddress])];
                        symbolName = [NSString stringWithFormat:@"[%@->%@]",
                                      [self findSymbolAtRVA:indirectAddress], symbolName];
                    } break;
                        
                    case INDIRECT_SYMBOL_ABS: {
                        [node.details appendRow:@"" col1:@"" col2:@"40000000" col3:@"INDIRECT_SYMBOL_ABS"];
            ...
            [node.details appendRow:@""
                               col1:@""
                               col2:@"Section"
                               col3:[NSString stringWithFormat:@"(%s,%s)",
                                     string(section->segname, 16).c_str(),
                                     string(section->sectname, 16).c_str()]];
            
            [node.details appendRow:@"" col1:@"" col2:@"Indirect Address" col3:[NSString stringWithFormat:@"0x%X ($+%u)", indirectAddress, indirectAddress - section->addr]];
...
- (MVNode *)createISymbols64Node:parent
    ...
    for (uint32_t nindsym = 0; nindsym < length / sizeof(uint32_t); ++nindsym) {
        ...
        while (--nsect > 0) {
            ...
            if ((indirectIndex & (INDIRECT_SYMBOL_LOCAL | INDIRECT_SYMBOL_ABS)) == 0) {
                ...
                switch (indirectIndex) {
                    case INDIRECT_SYMBOL_LOCAL: {
                        ...
                        [node.details appendRow:@"" col1:@"" col2:@"Target" col3:(symbolName = [self findSymbolAtRVA64:targetAddress])];
                        symbolName = [NSString stringWithFormat:@"[%@->%@]",
                                      [self findSymbolAtRVA64:indirectAddress], symbolName];
                    } break;
            ...
            [node.details appendRow:@"" col1:@"" col2:@"Indirect Address" col3:[NSString stringWithFormat:@"0x%qX ($+%qu)", indirectAddress, indirectAddress - section_64->addr]];
...
- (MVNode *)createTOCNode:parent
    ...
    while (NSMaxRange(range) < location + length) {
        ...
        [dataController read_uint32:range lastReadHex:&lastReadHex];
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Module" col3:NSSTRING(strtab + modules.at(dylib_table_of_contents->module_index)->module_name)];
...
- (MVNode *)createTOC64Node:parent
    ...
    while (NSMaxRange(range) < location + length) {
        ...
        [dataController read_uint32:range lastReadHex:&lastReadHex];
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Module" col3:NSSTRING(strtab + modules_64.at(dylib_table_of_contents->module_index)->module_name)];
...
- (MVNode *)createModulesNode:parent
    ...
    while (NSMaxRange(range) < location + length) {
        ...
        [dataController read_uint32:range lastReadHex:&lastReadHex];
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Module" col3:moduleName];
        
        [dataController read_uint32:range lastReadHex:&lastReadHex];
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Ext Defined Symbols Index" col3:[NSString stringWithFormat:@"%u", dylib_module->iextdefsym]];
        
        [dataController read_uint32:range lastReadHex:&lastReadHex];
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Ext Defined Symbols Number" col3:[NSString stringWithFormat:@"%u", dylib_module->nextdefsym]];
        
        [dataController read_uint32:range lastReadHex:&lastReadHex];
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Ext References Index" col3:[NSString stringWithFormat:@"%u", dylib_module->irefsym]];
        
        [dataController read_uint32:range lastReadHex:&lastReadHex];
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Ext References Number" col3:[NSString stringWithFormat:@"%u", dylib_module->nrefsym]];
        
        [dataController read_uint32:range lastReadHex:&lastReadHex];
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Local Symbols Index" col3:[NSString stringWithFormat:@"%u", dylib_module->ilocalsym]];
        
        [dataController read_uint32:range lastReadHex:&lastReadHex];
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Local Symbols Number" col3:[NSString stringWithFormat:@"%u", dylib_module->nlocalsym]];
        
        [dataController read_uint32:range lastReadHex:&lastReadHex];
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Ext Relocs Index" col3:[NSString stringWithFormat:@"%u", dylib_module->iextrel]];
        
        [dataController read_uint32:range lastReadHex:&lastReadHex];
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Ext Relocs Number" col3:[NSString stringWithFormat:@"%u", dylib_module->nextrel]];
        
        [dataController read_uint32:range lastReadHex:&lastReadHex];
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Init Pointers Index" col3:[NSString stringWithFormat:@"%u", dylib_module->iinit_iterm & 0xfff]];
        
        [node.details appendRow:@"" col1:@"" col2:@"Term Pointers Index" col3:[NSString stringWithFormat:@"%u", (dylib_module->iinit_iterm >> 16) & 0xffff]];
        
        [dataController read_uint32:range lastReadHex:&lastReadHex];
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Init Pointers Number" col3:[NSString stringWithFormat:@"%u", dylib_module->ninit_nterm & 0xffff]];
        
        [node.details appendRow:@"" col1:@"" col2:@"Term Pointers Number" col3:[NSString stringWithFormat:@"%u", (dylib_module->ninit_nterm >> 16) & 0xffff]];
        
        [dataController read_uint32:range lastReadHex:&lastReadHex];
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Module Info Address" col3:[NSString stringWithFormat:@"0x%X", dylib_module->objc_module_info_addr]];
        
        [dataController read_uint32:range lastReadHex:&lastReadHex];
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Module Info Size" col3:[NSString stringWithFormat:@"%u", dylib_module->objc_module_info_size]];
        
        [node.details setAttributesFromRowIndex:bookmark firstArg:MVMetaDataAttributeName, moduleName, nil];
        [node.details setAttributes:MVUnderlineAttributeName, @"YES", nil];
    }
...
- (MVNode *)createModules64Node:parent
    ...
    while (NSMaxRange(range) < location + length) {
        ...
        [dataController read_uint32:range lastReadHex:&lastReadHex];
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Ext Defined Symbols Index" col3:[NSString stringWithFormat:@"%u", dylib_module_64->iextdefsym]];
        
        [dataController read_uint32:range lastReadHex:&lastReadHex];
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Ext Defined Symbols Number" col3:[NSString stringWithFormat:@"%u", dylib_module_64->nextdefsym]];
        
        [dataController read_uint32:range lastReadHex:&lastReadHex];
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Ext References Index" col3:[NSString stringWithFormat:@"%u", dylib_module_64->irefsym]];
        
        [dataController read_uint32:range lastReadHex:&lastReadHex];
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Ext References Number" col3:[NSString stringWithFormat:@"%u", dylib_module_64->nrefsym]];
        
        [dataController read_uint32:range lastReadHex:&lastReadHex];
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Local Symbols Index" col3:[NSString stringWithFormat:@"%u", dylib_module_64->ilocalsym]];
        
        [dataController read_uint32:range lastReadHex:&lastReadHex];
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Local Symbols Number" col3:[NSString stringWithFormat:@"%u", dylib_module_64->nlocalsym]];
        
        [dataController read_uint32:range lastReadHex:&lastReadHex];
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Ext Relocs Index" col3:[NSString stringWithFormat:@"%u", dylib_module_64->iextrel]];
        
        [dataController read_uint32:range lastReadHex:&lastReadHex];
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Ext Relocs Number" col3:[NSString stringWithFormat:@"%u", dylib_module_64->nextrel]];
        
        [dataController read_uint32:range lastReadHex:&lastReadHex];
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Init Pointers Index" col3:[NSString stringWithFormat:@"%u", dylib_module_64->iinit_iterm & 0xfff]];
        
        [node.details appendRow:@"" col1:@"" col2:@"Term Pointers Index" col3:[NSString stringWithFormat:@"%u", (dylib_module_64->iinit_iterm >> 16) & 0xffff]];
        
        [dataController read_uint32:range lastReadHex:&lastReadHex];
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Init Pointers Number" col3:[NSString stringWithFormat:@"%u", dylib_module_64->ninit_nterm & 0xffff]];
        
        [node.details appendRow:@"" col1:@"" col2:@"Term Pointers Number" col3:[NSString stringWithFormat:@"%u", (dylib_module_64->ninit_nterm >> 16) & 0xffff]];
        
        [dataController read_uint32:range lastReadHex:&lastReadHex];
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Module Info Address" col3:[NSString stringWithFormat:@"0x%llX", dylib_module_64->objc_module_info_addr]];
        
        [dataController read_uint64:range lastReadHex:&lastReadHex];
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Module Info Size" col3:[NSString stringWithFormat:@"%u", dylib_module_64->objc_module_info_size]];
...
- (MVNode *)createTwoLevelHintsNode:parent
    ...
    while (NSMaxRange(range) < location + length) {
        ...
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Subimage" col3:[NSString stringWithFormat:@"%u", twolevel_hint->isub_image]];
        ...
        [node.details appendRow:@"" col1:@"" col2:@"TOC Index" col3:[NSString stringWithFormat:@"%u %@", twolevel_hint->itoc, symbolName]];
...
- (MVNode *)createSplitSegmentNode:parent
    ...
    while (NSMaxRange(range) < location + length) {
        ...
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"kind" col3:kind == 1 ? @"32-bit pointer" : kind == 2 ? @"64-bit pointer" : kind == 3 ? @"ppc hi16" : kind == 4 ? @"32-bit offset to IMPORT" : kind == 5 ? @"thumb2 movw" : kind == 6 ? @"ARM movw" : kind == 0x10 ? @"thumb2 movt low high 4 bits=0" : kind == 0x11 ? @"thumb2 movt low high 4 bits=1" : kind == 0x12 ? @"thumb2 movt low high 4 bits=2" : kind == 0x13 ? @"thumb2 movt low high 4 bits=3" : kind == 0x14 ? @"thumb2 movt low high 4 bits=4" : kind == 0x15 ? @"thumb2 movt low high 4 bits=5" : kind == 0x16 ? @"thumb2 movt low high 4 bits=6" : kind == 0x17 ? @"thumb2 movt low high 4 bits=7" : kind == 0x18 ? @"thumb2 movt low high 4 bits=8" : kind == 0x19 ? @"thumb2 movt low high 4 bits=9" : kind == 0x1A ? @"thumb2 movt low high 4 bits=0xA" : kind == 0x1B ? @"thumb2 movt low high 4 bits=0xB" : kind == 0x1C ? @"thumb2 movt low high 4 bits=0xC" : kind == 0x1D ? @"thumb2 movt low high 4 bits=0xD" : kind == 0x1E ? @"thumb2 movt low high 4 bits=0xE" : kind == 0x1f ? @"thumb2 movt low high 4 bits=0xF" : @"???"];
        ...
        do {
            ...
            [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                                   col1:lastReadHex
                                   col2:@"uleb128"
                               col3:[NSString stringWithFormat:@"%@ %@", [self is64bit] == NO ? [self findSectionContainsRVA:address] : [self findSectionContainsRVA64:address], (symbolName = [self is64bit] == NO ? [self findSymbolAtRVA:(uint32_t)address] : [self findSymbolAtRVA64:address])]];
...
- (MVNode *)createFunctionStartsNode:parent
    ...
    while (NSMaxRange(range) < location + length) {
        ...
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"uleb128" col3:(symbolName = [self is64bit] == NO ? [self findSymbolAtRVA:(uint32_t)address] : [self findSymbolAtRVA64:address])];
...
- (MVNode *)createDataInCodeEntriesNode:parent
    ...
    while (NSMaxRange(range) < location + length) {
        ...
        [dataController read_uint32:range lastReadHex:&lastReadHex];
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Offset" col3:[self findSymbolAtRVA:[self fileOffsetToRVA:data_in_code_entry->offset + imageOffset]]];
        
        [dataController read_uint16:range lastReadHex:&lastReadHex];
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Length" col3:[NSString stringWithFormat:@"%u", (uint32_t)data_in_code_entry->length]];
        
        [dataController read_uint16:range lastReadHex:&lastReadHex];
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location] col1:lastReadHex col2:@"Kind" col3:data_in_code_entry->kind == DICE_KIND_DATA ? @"DICE_KIND_DATA" : data_in_code_entry->kind == DICE_KIND_JUMP_TABLE8 ? @"DICE_KIND_JUMP_TABLE8" : data_in_code_entry->kind == DICE_KIND_JUMP_TABLE16 ? @"DICE_KIND_JUMP_TABLE16" : data_in_code_entry->kind == DICE_KIND_JUMP_TABLE32 ? @"DICE_KIND_JUMP_TABLE32" : data_in_code_entry->kind == DICE_KIND_ABS_JUMP_TABLE32 ? @"DICE_KIND_ABS_JUMP_TABLE32" : @"???"];
