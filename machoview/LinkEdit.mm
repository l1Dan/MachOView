                ...
                baseAddress:(uint32_t)baseAddress // start of the section containing the relocation (image: start of the first segment)
{
    ...
    for (uint32_t nreloc = 0; nreloc < length / sizeof(struct relocation_info); ++nreloc) {
        ...
        [node.details appendRow:@""
                           col1:@""
                           col2:@"Length"
                           col3:[NSString stringWithFormat:@"%u", relocation_info ? (1 << relocation_info->r_length) : (
                                   1 << scattered_relocation_info->r_length)]];
    ...
    for (uint32_t nsym = 0; nsym < length / sizeof(struct nlist); ++nsym) {
        ...
        if (nlist->n_type & N_STAB) {
            ...
            if (nlist->n_type & N_EXT)
                color = ((nlist->n_type & N_TYPE) == N_UNDF || (nlist->n_type & N_TYPE) == N_PBUD
                        ? [NSColor greenColor]
                        : [NSColor orangeColor]);
        }
    ...
    for (uint32_t nsym = 0; nsym < length / sizeof(struct nlist_64); ++nsym) {
        ...
        if (nlist_64->n_type & N_STAB) {
            ...
            if (nlist_64->n_type & N_EXT)
                color = ((nlist_64->n_type & N_TYPE) == N_UNDF || (nlist_64->n_type & N_TYPE) == N_PBUD
                        ? [NSColor greenColor]
                        : [NSColor orangeColor]);
        }
        ...
        if ((nlist_64->n_desc & N_WEAK_DEF) == N_WEAK_DEF)
            [node.details appendRow:@""
                               col1:@""
                               col2:@"0080"
...
- (MVNode *)createModulesNode:parent
    ...
    while (NSMaxRange(range) < location + length) {
        ...
        [node.details appendRow:@"" col1:@"" col2:@"Term Pointers Index" col3:[NSString stringWithFormat:@"%u", (
                dylib_module->iinit_iterm >> 16) & 0xffff]];
        ...
        [node.details appendRow:@"" col1:@"" col2:@"Term Pointers Number" col3:[NSString stringWithFormat:@"%u", (
                dylib_module->ninit_nterm >> 16) & 0xffff]];
...
- (MVNode *)createModules64Node:parent
    ...
    while (NSMaxRange(range) < location + length) {
        ...
        [node.details appendRow:@"" col1:@"" col2:@"Term Pointers Index" col3:[NSString stringWithFormat:@"%u", (
                dylib_module_64->iinit_iterm >> 16) & 0xffff]];
        ...
        [node.details appendRow:@"" col1:@"" col2:@"Term Pointers Number" col3:[NSString stringWithFormat:@"%u", (
                dylib_module_64->ninit_nterm >> 16) & 0xffff]];
