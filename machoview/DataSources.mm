...
- (void)tableView:(NSTableView *)aTableView setObjectValue:(id)anObject forTableColumn:(NSTableColumn *)aTableColumn row:(NSInteger)rowIndex {
    ...
    if (selectedNode.details != nil) {  // option1: plain hex value
        MVRow *row = [selectedNode.details getRowToDisplay:rowIndex];
        if (row == nil) {
            return;
        }
        ...
        [selectedNode.details setAttributesForRowIndex:rowIndex firstArg:MVTextColorAttributeName, [NSColor redColor], nil];
